...
<div class="grid grid-cols-3 gap-4">
        ...
        <ul class="flex flex-col">
            ...
            @foreach ($course->posts as $post)
                <li class="flex items-center text-gray-600 mt-2">
                    {{ $post->name }}
                    @if ($post->free)
                        <span class="text-xs text-gray-500 font-semibold bg-gray-300 rounded-full px-2 ml-auto">Gratis</span>
                    @endif
                </li>    
            @endforeach
        </ul>
    ...
    <div class="text-gray-700 col-span-2">
        ...
        <div class="flex mt-3">
            <img class="h-10 w-10 rounded-full mr-2" src="{{ $course->user->avatar }}" alt="user avatar">
            <div>
